...
export async function generarPDF(req, res) {
  ...
  try {
    ...
    await page.goto(`http://localhost:3000/ticket-html/${idTicket}`, {
  ...
  app.post("/alta", upload.single("imagen"), async (req, res) => {
    try {
      ...
      const nuevoProducto = {
        ...
        img: rutaImg,
        url: datos.url || "",
        tipoBotin: datos.tipo === "Botin" ? datos.tipoBotin : null,
        tipoZapatilla: datos.tipo === "Zapatilla" ? datos.tipoZapatilla : null,
        largoTapones: datos.tipo === "Botin" ? datos.tamanoTapones : null,
        activo: 1,
      };
